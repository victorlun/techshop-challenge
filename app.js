...
function initLoan() {
  ...
  while (isNaN(loanAmount) || !loanAmount || loanAmount == "")
    loanAmount = prompt(
      "Get a loan, but please enter a number.  To cancel, enter '0' "
    );
  while (loanAmount > bank.getBalance() * 2)
    loanAmount = prompt(
      "You can't loan more than 2x your balance. Enter a new loan."
    );
  if (bank.getDebt() > 0) {
    alert(
      "You already have a loan, pay that back in full to apply for a new loan!"
    );
  } else {
...
function repayLoan() {
  if (work.getPay() === 0) {
    alert(
      "You don't have any money to pay back your loan with, work a little! ⚡"
    );
...
function showLaptop() {
  ...
  const selectedLaptop = laptops.find(
    (laptop) => laptop.id === selectedLaptopId
  );
